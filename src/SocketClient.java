...
public class SocketClient {
	...
	private final static String SEND_FILE = "sendfile";
	
	
	private static void writeFile(String[] fileMetadata) {
		try (PrintWriter writer=new PrintWriter(fileMetadata[0])){
			writer.print(fileMetadata[1]);
		}catch(Exception ex) {
			log.log(Level.INFO, "Error While Writing File");
		}
	}
	private static Payload buildMessage(String message,boolean isFile) {
		Payload payload = new Payload();
		payload.setPayloadType(isFile?PayloadType.FILE:PayloadType.MESSAGE);
	...
	private static void processPayload(Payload p) {
		...
		case FILE:
			if (event != null) {
				// fileMetadata contains filename--fe--filecontent
				String[] fileMetadata=p.getMessage().split("--fe--");
				writeFile(fileMetadata);
				event.onFileReceive(p.getClientName(), fileMetadata[0]);
			}
			break;
		default:
	...
	private static boolean isClientSendFile(String message) {
		if (message.indexOf(Room.COMMAND_TRIGGER) > -1) 
			return message.split(Room.COMMAND_TRIGGER)[1].split(" ")[0].equalsIgnoreCase(SEND_FILE);
		return false;
	}
	
	private static String extractFileNameFromPath(String fullPath) {
		return fullPath.substring(fullPath.lastIndexOf("\\")+1);
	}
	public static void sendMessage(String message) {
		Payload payload=null;
		if(isClientSendFile(message)) {
			int sep_index=message.split(Room.COMMAND_TRIGGER)[1].indexOf(" ");
			String fileName=message.substring(sep_index+1).trim();
			...
			try {
				fileMetadata=extractFileNameFromPath(fileName)+"--fe--"+new String(Files.readAllBytes(Paths.get(fileName)));
			} catch (IOException e) {
				log.log(Level.SEVERE, "Error While Reading File");
				e.printStackTrace();
			}
			payload=buildMessage(fileMetadata,true); // true indicates that payload contains file
		}else {
			payload=buildMessage(message,false);
		}
		sendPayload(payload);
	}
